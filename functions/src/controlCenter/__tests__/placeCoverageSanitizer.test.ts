...
import {sanitizeCoverageCell} from "../placeCoverageSanitizer";
...
test("sanitizes live area cache into aggregate coverage only", () => {
  ...
  assert.equal(result.cell_id, "w2839");
...
test("marks missing or older-than-24h cell snapshots stale", () => {
  const now = Date.UTC(2026, 7, 14, 0, 0, 0);

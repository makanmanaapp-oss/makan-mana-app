...
function priceLevelFromRange(value: unknown): number {
  const p = (optionalText(value, 80) ?? "").toLowerCase();
  if (!p) return 0;
...
function curatedAreaCandidate(input: {
  ...
  return {
    ...
    name: input.name,
    cuisine: input.primaryCategory ?? "Makanan",
    emoji: "🍽️",
    // Unknown provider-derived values stay unknown/neutral. Mobile deliberately
    // hides rating 0; Algorithm 2 treats zero rating as unknown, not bad data.
    rating: 0,
    userRatingCount: 0,
    priceLevel: priceLevelFromRange(input.priceRange),
    distanceKm: 0, // recalculated by AreaCandidatePool exact-radius projection
    isOpen: input.businessStatus !== "temporarily_closed" && input.businessStatus !== "permanently_closed",
    address: input.address ?? "",
    matchScore: 0,
    matchReasonKeys: [],
    negativeSignals: ["rating_unverified"],
    priceEstimate: input.priceRange ?? "",
    photoUrl: input.coverImageUrl,
    openingPeriods: null,
    lat: input.latitude,
    lng: input.longitude,
  };
...
async function publishMasterPlace(resourceId: string, payload: Plain, requestId: string): Promise<Plain> {
  ...
  const areaCellRef = db.collection(C_AREA).doc(areaCellId);

  const result = await db.runTransaction(async (tx) => {
    const [registrySnap, headSnap, existingPublication, areaCellSnap] = await Promise.all([
      tx.get(registryRef),
      tx.get(headRef),
    ...
    const mediaGallery = arrayValue(payload.mediaGallery, 200);
    ...
    const curated: Plain = {
      ...
      // Menu is first-class curated data. Public Restaurant Profile V2 reads it
      // from the active publication; never leave it trapped in Supabase only.
      menuItems,
      priceRange,
    ...
    // Put first-party curated places into the permanent area candidate database
    // immediately. This is the missing path that makes a Control Center-only
    // restaurant discoverable without waiting for Google/provider discovery.
    const existingAreaCandidates = Array.isArray(areaCellSnap.data()?.candidates)
      ? areaCellSnap.data()!.candidates as PlaceCandidate[]
      : [];
    const newAreaCandidate = curatedAreaCandidate({
      canonicalPlaceId,
      name,
      address,
      latitude,
    ...
    const preserved = existingAreaCandidates.filter((candidate) =>
      candidate.placeId !== canonicalPlaceId && candidate.canonicalPlaceId !== canonicalPlaceId);
    tx.set(areaCellRef, {
      cellId: areaCellId,
      candidates: [newAreaCandidate, ...preserved].slice(0, MAX_CANDIDATES_PER_CELL),
      updatedAt: now,
    }, {merge: true});
    ...
    return {canonicalPlaceId, publicationId, versionNumber, idempotent: false, areaCellId};
  });

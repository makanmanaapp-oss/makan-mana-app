...
} from "../domain/places/canonical/masterCandidateSeed";
...
async function publishMasterPlace(resourceId: string, payload: Plain, requestId: string): Promise<Plain> {
  ...
  const cellId = storageCellForPlace(latitude, longitude);
  const cellRef = db.collection(AREA_CACHE).doc(cellId);
  ...
  const curated: Plain = {
    ...
    menuItems: arrayValue(payload.menuItems),
    priceRange,
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    // normal provider gap discovery.
    tx.set(cellRef, {
      cellId,
      candidates: seeded,
      updatedAt: now,
      controlCenterCanonicalSeedAt: now,
    }, {merge: true});
    ...
    tx.create(publicationRef, {
      ...
      controlCenterRequestId: requestId,
      sourceCanonicalVersion: "control-center-master-v2-candidate-priority",

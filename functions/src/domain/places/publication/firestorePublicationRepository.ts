...
import { PlaceStatusAuditEntry, statusAuditId } from "./publicationAudit";
import {
...
export interface PublicationFirestoreClock {
  now(): EpochMillis;
}

export class FirestorePublicationStore
  implements
    PlacePublicationRepository,
    PlaceRollbackRepository,
    PlaceStatusAuditRepository,
    PlaceCacheInvalidationRepository
{
  constructor(
    ...
    private clock: PublicationFirestoreClock = { now: () => Date.now() },
  ) {}
  ...
  async createPublicationVersion(
    ...
    if (existing.exists) return existing.data() as PlacePublicationVersion;
    ...
    if (!sameContent.empty) {
      return sameContent.docs[0].data() as PlacePublicationVersion;
    }
  ...
  async listVersionsByPlace(
    ...
    return {
      items: docs.map((d) => d.data() as PlacePublicationVersion),
  ...
  async nextVersionNumber(placeId: string): Promise<number> {
    ...
    return ((snap.docs[0].data() as PlacePublicationVersion).versionNumber ?? 0) + 1;
  }
  ...
  async getActiveHead(placeId: string): Promise<PlacePublicationHead | null> {
    ...
    return s.exists ? (s.data() as PlacePublicationHead) : null;
  }
  ...
  async requestRollback(params: {
    ...
    const existing = await ref.get();
    if (existing.exists) return existing.data() as PlacePublicationRollback;
  ...
  async getRollback(rollbackId: string): Promise<PlacePublicationRollback | null> {
    ...
    return s.exists ? (s.data() as PlacePublicationRollback) : null;
  }
  ...
  async approveRollback(
    ...
    if (r.status === "approved") return r; // idempoten
  ...
  async executeRollbackInEmulator(
    ...
    const r = await this.getRollback(rollbackId);
    if (!r) throw new Error(`rollback not found: ${rollbackId}`);
    if (r.status === "executed_in_emulator") return r; // idempoten
    ...
    const snapshot: PlacePublicationSnapshot = toPlain(target.snapshot);
    const contentInput = {
      placeId: r.placeId,
      snapshot,
  ...
  async listRollbacksByPlace(
    ...
    const docs = snap.docs.slice(0, limit);
    return {
      items: docs.map((d) => d.data() as PlacePublicationRollback),
      nextCursor: snap.docs.length > limit ? docs[docs.length - 1].id : undefined,
    };
  }
  ...
  async appendStatusAudit(entry: PlaceStatusAuditEntry): Promise<PlaceStatusAuditEntry> {
    ...
    const existing = await ref.get();
    if (existing.exists) return existing.data() as PlaceStatusAuditEntry;
  ...
  async listStatusAudit(
    ...
    const docs = snap.docs.slice(0, limit);
    return {
      items: docs.map((d) => d.data() as PlaceStatusAuditEntry),
      nextCursor: snap.docs.length > limit ? docs[docs.length - 1].id : undefined,
    };
  }
  ...
  async appendInvalidationEvent(
    ...
    const existing = await ref.get();
    if (existing.exists) return existing.data() as PlaceCacheInvalidationEvent;
  ...
  async listInvalidationEvents(
    ...
    const docs = snap.docs.slice(0, limit);
    return {
      items: docs.map((d) => d.data() as PlaceCacheInvalidationEvent),
      nextCursor: snap.docs.length > limit ? docs[docs.length - 1].id : undefined,
    };
  }
}

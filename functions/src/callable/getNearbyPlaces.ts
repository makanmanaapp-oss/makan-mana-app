...
export const getNearbyPlaces = onCall(
  ...
  async (request) => {
    ...
    // Keep Explore/Home Nearby on the SAME persistent area pool as Spin. Published
    // Control Center places are inserted into area_place_cache by the master-place
    // bridge, so a curated place can be discovered even when Google never returned
    // it. This remains rollout + emergency-legacy gated.
    ...
    if (apiKey) {
      try {
        if (areaCoverageOn) {
          ...
          if (area.pool.candidates.length > 0) {
            candidates = area.pool.candidates;
            source = "area_registry";
          } else {
            candidates = await searchNearby({lat, lng, radiusMeters: radiusM, languageCode, apiKey});
          }

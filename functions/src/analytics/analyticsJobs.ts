...
async function applyMetric(params: {
  ...
  return db.runTransaction(async (tx) => {
    ...
    if (params.sourceEventRef) {
      tx.set(params.sourceEventRef, {analyticsAppliedAtMs: now}, {merge: true});
    }
...
export const aggregateAnalyticsEventOnCreate = onDocumentCreated(
  ...
  async (event) => {
    ...
    const placeId = text(candidate.placeId);
...
export const aggregateRestaurantFollowOnCreate = onDocumentCreated(
  ...
  async (event) => {
    ...
    const placeId = text(data.canonicalPlaceId);
    if (!placeId) return;
    await applyMetric({
      canonicalPlaceId: placeId,
...
export const aggregateMenuCommentOnCreate = onDocumentCreated(
  ...
  async (event) => {
    const data = event.data?.data() ?? {};
    const placeId = text(data.canonicalPlaceId) || text(data.placeId);
    if (!placeId) return;
    await applyMetric({
      canonicalPlaceId: placeId,
      dayKey: businessDayKey(Date.now()),
...
export async function reconcileAnalyticsDay(dayKey: string, nowMs: number): Promise<number> {
  ...
  const events: RawAnalyticsEvent[] = snap.docs.map((doc) => {
    const raw = doc.data() ?? {};
    return {
      eventId: doc.id,
      eventType: raw.eventType,

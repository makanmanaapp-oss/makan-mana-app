...
exports.SAFETY_WARNING_STATES = exports.BUSINESS_DISPLAY_STATES = exports.RATING_DISPLAY_STATES = exports.PRICE_DISPLAY_STATES_V16 = exports.HOURS_DISPLAY_STATES = void 0;
exports.deriveHoursDisplayState = deriveHoursDisplayState;
exports.derivePriceDisplayState = derivePriceDisplayState;
exports.deriveRatingDisplayState = deriveRatingDisplayState;
exports.deriveBusinessDisplayState = deriveBusinessDisplayState;
exports.deriveSafetyWarningState = deriveSafetyWarningState;
// ---------------------------------------------------------------------------
// Keadaan paparan kanonikal (ID bebas bahasa; label l10n pada Phase 1.9)
// ---------------------------------------------------------------------------
exports.HOURS_DISPLAY_STATES = [
    "hours_known",
    "hours_unknown",
    "hours_expired",
    "temporarily_closed",
...
exports.PRICE_DISPLAY_STATES_V16 = [
    "price_verified",
    "estimated_price",
    "price_unknown",
    "price_expired",
];
exports.RATING_DISPLAY_STATES = [
    "rating_shown",
    "rating_hidden",
    "rating_stale",
];
exports.BUSINESS_DISPLAY_STATES = [
    "operating",
    "temporarily_closed",
    "moved",
 ...
 */
function deriveHoursDisplayState(hours, status, hoursFreshness) {
 ...
 */
function derivePriceDisplayState(commercial, priceFreshness) {
 ...
 */
function deriveRatingDisplayState(quality, ratingFreshness) {
    const hasRating = typeof quality.rating === "number" && Number.isFinite(quality.rating);
    const hasCount = typeof quality.reviewCount === "number" && Number.isFinite(quality.reviewCount);
    if (!hasRating)
        return { state: "rating_hidden", warningCode: "rating_missing" };
    if (!hasCount)
        return { state: "rating_hidden", warningCode: "review_count_missing" };
 ...
 */
function deriveBusinessDisplayState(status, businessStatusFreshness) {
    if (status === "permanently_closed") {
        return {
            state: "blocked",
 ...
 */
function deriveSafetyWarningState(safety, halalFreshness, allergenFreshness, dietaryFreshness) {
    const warningCodes = [];
    let halal = HALAL_DISPLAY[safety.halal.state];
    ...
    if (halal === "halal_unknown")
        warningCodes.push("halal_unknown");
    const allergenMissing = safety.allergenEvidenceLevel === "unknown" || safety.allergenReported.length === 0;
    const allergenUnknown = allergenMissing || allergenFreshness?.expired === true;
    if (allergenUnknown)
        warningCodes.push("allergen_evidence_unknown");
    else if (allergenFreshness?.stale === true)
        warningCodes.push("allergen_evidence_stale");
    const dietaryUnknown = safety.dietaryReported.length === 0 || dietaryFreshness?.expired === true;
    if (dietaryUnknown)
        warningCodes.push("dietary_evidence_unknown");
    return {
        halal,
        allergenUnknown,
        dietaryUnknown,

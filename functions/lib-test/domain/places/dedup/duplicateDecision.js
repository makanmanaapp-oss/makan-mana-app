...
exports.DUPLICATE_DECISIONS = void 0;
exports.scoreBand = scoreBand;
exports.evaluateDuplicateDecision = evaluateDuplicateDecision;
/** Phase 1.4 — band keputusan + peraturan khas (safety-first). */
const config_1 = require("./config");
const duplicateScoring_1 = require("./duplicateScoring");
exports.DUPLICATE_DECISIONS = [
    "exact_duplicate",
    "auto_link_source",
    "review_required",
    "possible_duplicate",
...
function scoreBand(score, config = config_1.DEFAULT_DEDUP_CONFIG) {
    if (score >= config.bands.exact)
        return "exact";
    if (score >= config.bands.review)
        return "review";
    if (score >= config.bands.possible)
        return "possible";
    return "separate";
}
/**
 ...
 */
function evaluateDuplicateDecision(signals, config = config_1.DEFAULT_DEDUP_CONFIG) {
    const score = (0, duplicateScoring_1.computeDuplicateScore)(signals, config).adjustedScore;
    const reasons = [];
    const warnings = [];
    const matchedBy = [];
    ...
    if (providerExact)
        matchedBy.push("provider_id");
    if (merchantExact)
        matchedBy.push("merchant_id");
    if (phoneMatch)
        matchedBy.push("phone");
    if (geoSim >= 0.85)
        matchedBy.push("geo");
    if (addrSim >= config.brandSameNameSimilarity)
    ...
    const hasNonNameStrong = phoneMatch || geoSim >= 0.85 || addrSim >= config.brandSameNameSimilarity ||

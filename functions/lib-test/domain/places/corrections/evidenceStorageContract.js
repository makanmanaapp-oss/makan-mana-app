"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.EVIDENCE_MODERATION_STATUSES = exports.EVIDENCE_LIMITS = exports.ALLOWED_EVIDENCE_MIME = void 0;
exports.validateEvidenceMetadata = validateEvidenceMetadata;
exports.validateEvidenceCount = validateEvidenceCount;
/**
 * Phase 1.14A — KONTRAK storan bukti pembetulan (SEDIA UNTUK MASA DEPAN).
 *
 * Muat naik bukti LANGSUNG belum dibina/di-deploy. Modul ini mentakrifkan
 * kontrak selamat + pengesah supaya rules Storan dan callable boleh disemak
 * lebih awal. TIADA I/O di sini.
 *
 * KESELAMATAN: hanya jenis imej disokong; had saiz/bilangan ketat; tiada URL
 * awam secara lalai; metadata sahaja disimpan (bukan kandungan mentah dalam
 * Firestore).
 */
exports.ALLOWED_EVIDENCE_MIME = [
    "image/jpeg",
    "image/png",
    "image/webp",
    "image/heic",
];
exports.EVIDENCE_LIMITS = {
    maxBytes: 8 * 1024 * 1024, // 8 MB setiap fail
    maxFilesPerSubmission: 5,
    minBytes: 64,
};
exports.EVIDENCE_MODERATION_STATUSES = [
    "pending_scan",
    "approved",
    "rejected",
    "quarantined",
];
/** Sahkan metadata SATU fail bukti terhadap kontrak. TULEN. */
function validateEvidenceMetadata(meta) {
    const errors = [];
    if (!meta.ownerUid)
        errors.push("owner_required");
    if (!meta.submissionId)
        errors.push("submission_required");
    if (!meta.storagePath || !meta.storagePath.startsWith(`corrections/${meta.submissionId ?? ""}/`)) {
        errors.push("invalid_storage_path");
    }
    if (!meta.contentType || !exports.ALLOWED_EVIDENCE_MIME.includes(meta.contentType)) {
        errors.push("unsupported_content_type");
    }
    if (meta.byteSize === undefined || meta.byteSize < exports.EVIDENCE_LIMITS.minBytes) {
        errors.push("file_too_small");
    }
    else if (meta.byteSize > exports.EVIDENCE_LIMITS.maxBytes) {
        errors.push("file_too_large");
    }
    if (!meta.checksumSha256 || meta.checksumSha256.length < 32)
        errors.push("invalid_checksum");
    return { valid: errors.length === 0, errors };
}
/** Sahkan bilangan fail per penghantaran. */
function validateEvidenceCount(count) {
    return count <= exports.EVIDENCE_LIMITS.maxFilesPerSubmission
        ? { valid: true, errors: [] }
        : { valid: false, errors: ["too_many_files"] };
}

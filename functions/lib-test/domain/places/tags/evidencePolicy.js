...
exports.EVIDENCE_POLICY_VERSION = void 0;
exports.defaultEvidenceLevelForSource = defaultEvidenceLevelForSource;
exports.evaluateEvidencePolicy = evaluateEvidencePolicy;
exports.EVIDENCE_POLICY_VERSION = "tag_evidence_policy_v1";
/** Aras bukti lalai dicadang bagi sumber (dokumentasi + fallback). */
function defaultEvidenceLevelForSource(source) {
    switch (source) {
...
function halalPolicy(ev, issues) {
    switch (ev.tagId) {
        case "certified":
...
function healthPolicy(ev, issues) {
 ...
 */
function evaluateEvidencePolicy(ev, tagDef, familyDef) {
    const issues = [];
    ...
    if (tagDef.safetySensitive &&
        ev.evidenceLevel === "inferred" &&
        !familyDef.allowedEvidenceLevels.includes("inferred")) {
        issues.push("safety_tag_cannot_be_inferred");
    }
    if (familyDef.familyId === "halal_evidence")
        halalPolicy(ev, issues);
    if (familyDef.familyId === "health")
        healthPolicy(ev, issues);
